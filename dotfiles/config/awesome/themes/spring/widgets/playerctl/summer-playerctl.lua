...
local function worker(user_args)
	...
	playerctl_widget = wibox.widget({
		{
			...
			shape = function(cr, w, h)
				gears.shape.partially_rounded_rect(cr, w, h, true, false, false, true, 8)
			end,
		...
		},
		{
			...
			fg = "#FFFFFF",
			shape = function(cr, w, h)
				gears.shape.partially_rounded_rect(cr, w, h, false, true, true, false, 8)
			end,
		},
	...
	-- })
	aweseom.connect_signal("metadata", function(_, title, artist, album_path, album, new, player_name)
		local _new = "not-new"
		if new == true then
			_new = "new"
		end
	...
	awesome.connect_signal("position", function(_, interval_sec, length_sec, player_name)
	...
	awesome.connect_signal("no_players", function()
		playerctl_widget:set_visible(false)
	end)
...
return setmetatable(playerctl_widget, {
	__call = function(_, ...)
		return worker(...)
	end,
})

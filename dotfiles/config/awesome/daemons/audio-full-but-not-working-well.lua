...
local function get_devices(self)
	awful.spawn.easy_async_with_shell(
		[[
(pactl -f json list sinks 2&> /dev/null; pactl -f json list sources 2&> /dev/null) |
  jq -r -c '.[] | [.index, .name, .description, .mute, .base_volume.value_percent, .properties["media.class"], .properties["device.class"] ]' |
  sed -e 's/"//g'  -e 's/%//g'
  ]],
		function(stdout)
			for data in stdout:gmatch("%b[]") do
				local device = {}
				local info = gstring.split(data:sub(2, -2), ",")
				if info[7] == "sound" then
					device.id = info[1]
					device.name = info[2]
					device.description = info[3] == "(null)" and nil or info[3]
					device.mute = (info[4] == "yes" and true) or false
					device.volume = tonumber(info[5])
					device.default = false

					local type = info[6]
					if type == "Audio/Sink" then
						device.type = "sinks"
					elseif type == "Audio/Source" then
						device.type = "sources"
					end
...
local function new()
	...
	awful.spawn.easy_async("pkill -f 'pactl subscribe'", function()
		awful.spawn.with_line_callback([[ pactl subscribe | grep --line-buffered "sink" ]], {

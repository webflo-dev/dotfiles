...
local actual_value = 100
local actual_muted = false
local first_run = false

local emit_mic_info = function()
	awful.spawn.easy_async_with_shell(
		"printf \"%s|%s\" $(pactl get-source-volume @DEFAULT_SOURCE@ | grep '^Volume:' | cut -d / -f 2 | tr -d ' ' | sed 's/%//') $(pactl get-source-mute @DEFAULT_SOURCE@ | cut -d: -f2 | tr -d ' ')",
		function(stdout)
			local values = gears.string.split(helperString.remove_linebreaks(stdout), "|")
			local value = values[1]
			local mute = values[2] == "yes" and true or false
			awesome.emit_signal("signal::mic", value, mute)
		end
	)
end

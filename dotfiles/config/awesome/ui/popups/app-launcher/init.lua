...
-- local base = require("ui.widgets.base")
	...
	function app.spawn()
		...
		if entry.terminal == true then
			if self.terminal ~= nil then
				...
				awful.spawn.easy_async("gtk4-launch " .. entry.filename, function(_, stderr)
					if stderr then
						awful.spawn(entry.executable)
					end
				end)
...
local function new(args)
	...
	ret._private.widget = awful.popup(tpl.popup({
		type = "dock",
